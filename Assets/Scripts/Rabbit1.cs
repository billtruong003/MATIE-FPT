...
{
    [SerializeField]
    private Transform rabbitPlace1;
    ...
    private void Update()
    {
        if (!locked)
        {
            ...
            {
                isDragging = false;
                if (Mathf.Abs(transform.position.x - rabbitPlace1.position.x) <= 0.5f &&
                    Mathf.Abs(transform.position.y - rabbitPlace1.position.y) <= 0.5f)
                {
                    // Nếu đối tượng nằm ở đúng vị trí đích, khóa nó
                    transform.position = new Vector3(rabbitPlace1.position.x, rabbitPlace1.position.y, 0);
                    locked = true;
                }
                else
                {
