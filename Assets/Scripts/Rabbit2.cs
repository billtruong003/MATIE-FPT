...
{
    [SerializeField]
    private Transform rabbitPlace2;
    ...
    private void Update()
    {
        if (!locked)
        {
            ...
            {
                isDragging = false;
                if (Mathf.Abs(transform.position.x - rabbitPlace2.position.x) <= 0.5f &&
                    Mathf.Abs(transform.position.y - rabbitPlace2.position.y) <= 0.5f)
                {
                    // Nếu đối tượng nằm ở đúng vị trí đích, khóa nó
                    transform.position = new Vector3(rabbitPlace2.position.x, rabbitPlace2.position.y, 0);
                    locked = true;
                }
                else
                {
